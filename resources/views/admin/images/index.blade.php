<x-app-layout>
   <div class="p-4 sm:ml-64">
                  ...
                  <div class="grid grid-cols-1 gap-4 md:grid-cols-6">
                     ...
                     <div class="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                        <div class="flex items-center justify-center h-32">
                            <img class="rounded-t-lg" src="{{ route('images.show', $image->id) }}" alt="{{ $image->name }}" style="width: 100%; height: 100%; object-fit: cover;" />
                        </div>
                        <div class="p-5">
                           <h5 class="mb-2 font-bold tracking-tight text-gray-900 text-l dark:text-white">{{ $image->name }}</h5>
                           <div>
                              <button class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-green-500 rounded-lg hover:bg-green-600 focus:ring-4 focus:outline-none focus:ring-green-300 dark:bg-green-600 dark:hover:bg-green-700 dark:focus-ring-green-800" onclick="copyImageTag('{{ $image->image }}', '{{ $image->name }}')">
                                 Copy
                              </button>
                              <button data-modal-target="edit-modal{{ $image->id }}" data-modal-toggle="edit-modal{{ $image->id }}" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover-bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark-bg-blue-600 dark-hover-bg-blue-700 dark-focus-ring-blue-800">
                                 Edit
                              </button>
     ...
     <div class="relative w-full max-w-md max-h-full">
         ...
         <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
             ...
             <div class="px-6 py-6 lg:px-8">
                 ...
                 <form class="space-y-6" action="{{ route('images.store') }}" method="POST" enctype="multipart/form-data">
                  @csrf
                  @method('POST')
                    <div>
    ...
    @foreach($images as $image)
    {{-- Modal Edit --}}
    <div id="edit-modal{{ $image->id }}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
     <div class="relative w-full max-w-md max-h-full">
         <!-- Modal content -->
         <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
             <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" data-modal-hide="edit-modal{{ $image->id }}">
                 <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                 <span class="sr-only">Close modal</span>
             </button>
             <div class="px-6 py-6 lg:px-8">
                 <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Edit Referensi</h3>
                 <form class="space-y-6" action="{{ route('images.update', $image->id) }}" method="POST" enctype="multipart/form-data">
                     @csrf
                     @method('PUT')
                     <div>
                        <label for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Nama Referensi</label>
                        <input type="text" name="name" id="name" value="{{ old('name', $image->name) }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" placeholder="Nama Perusahaan" required>
                     </div>
                     <div>
     ...
     <div class="relative w-full max-w-md max-h-full">
         <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
             ...
             <div class="p-6 text-center">
                 ...
                 <form action="{{ route('images.destroy', $image->id) }}" method="POST" class="inline-block">
                  @csrf
                  @method('delete')
